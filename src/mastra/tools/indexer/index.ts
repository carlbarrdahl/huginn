...
const indexer = createIndexer(
  process.env.CURATOR_INDEXER_URL!,
  process.env.CURATOR_TENANT,
);
...
export const listStrategies = createTool({
  id: "list-strategies",
  description: "List funding strategies from the indexer, optionally filtered by owner.",
  ...
  execute: async ({ owner, limit }) => {
    ...
    return { strategies: result?.items ?? [], totalCount: result?.totalCount ?? 0 };
  },
});

export const getStrategy = createTool({
  id: "get-strategy",
  description: "Get full details for a single strategy.",
  inputSchema: z.object({
    strategyAddress: z.string().describe("Strategy contract address"),
  }),
  outputSchema: z.object({ strategy: strategySchema.nullable() }),
...
export const strategyBalances = createTool({
  ...
  description: "Get token balances for a strategy.",
  inputSchema: z.object({
    strategyAddress: z.string().describe("Strategy contract address"),
  }),
  outputSchema: z.object({
...
export const listDistributions = createTool({
  ...
  description: "List past distributions for a strategy.",
  inputSchema: z.object({
    strategyAddress: z.string().describe("Strategy contract address"),
    limit: z.number().default(10),
  }),
  ...
  execute: async ({ strategyAddress, limit }) => {
    ...
    return { distributions: result?.items ?? [], totalCount: result?.totalCount ?? 0 };
  },
});
